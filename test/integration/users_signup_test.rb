...
class UsersSignupTest < ActionDispatch::IntegrationTest
  ...
  test "non-activated users should not appear in users#show" do
    get signup_path
    assert_difference 'User.count', 1 do
      post signup_path, params: { user: { name:  "Example User",
                                          email: "user@example.com",
    ...
    user = assigns(:user)
    assert_not user.activated?
    # Assert that users#show redirects to root for non-activated user
    log_in_as(users(:michael))
    get user_path(user)
    assert_redirected_to root_url
    # Assert that users#show works after account activation
    get edit_account_activation_url(user.activation_token, email: user.email)
    assert user.reload.activated?
    get user_path(user)
    assert_response :success
